function loadSnakeGame(container) {
    ...
    // --- GAME STATE ---
    let snake, food, bonusFood, direction, pendingDirection, score, level, speed, isGameOver, isPaused, gameLoopTimeout, bonusFoodTimeout, bonusFoodSpawnTime;
    let speedIncreaseCounter = 0, currentFoodColorIndex = 0, highScores = [];
    let audio = { eatPool: [] };

    // --- DOM ELEMENTS ---
    let canvas, ctx, uiContainer, scoreDisplay, levelDisplay, highScoresDisplay, overlay;

    // --- INITIALIZATION ---
    function init() {
        createDOM(); preloadAudio(); loadHighScores(); updateHighScoresDisplay(); showStartScreen(); setupEventListeners();
    }
    function createDOM() {
        const gameContainer = document.createElement('div');
        gameContainer.style.position = 'relative'; gameContainer.style.display = 'flex';
        gameContainer.style.flexDirection = 'column'; gameContainer.style.alignItems = 'center'; gameContainer.style.gap = '15px';
        ...
        uiContainer.append(scoreLabel, scoreDisplay, ' | ', levelLabel, levelDisplay); gameContainer.appendChild(uiContainer);
        ...
        const style = document.createElement('style'); style.textContent = `
            ...
            #overlay { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; visibility: hidden; opacity: 0; transition: opacity 0.3s, visibility 0.3s; }
    ...
    function preloadAudio() { for (let i = 0; i < EAT_SOUND_POOL_SIZE; i++) { audio.eatPool.push(new Audio(SOUND_EAT_SRC)); } audio.eatPoolIndex = 0; audio.bonus = new Audio(SOUND_BONUS_EAT_SRC); audio.gameOver = new Audio(SOUND_GAME_OVER_SRC); }
    ...
    function setupGame() { isGameOver = false; isPaused = false; score = 0; speedIncreaseCounter = 0; currentFoodColorIndex = 0; let levelSelect = document.getElementById('level-select'); level = parseInt(levelSelect.value); speed = 150 - (level * 10); snake = [{ x: Math.floor(TILE_COUNT / 2), y: Math.floor(TILE_COUNT / 2), color: SNAKE_DEFAULT_COLOR }]; direction = { x: 0, y: 0 }; pendingDirection = null; placeFood(); bonusFood = null; clearTimeout(gameLoopTimeout); canvas.style.setProperty('--border-color', BORDER_PULSE_COLORS[0]); canvas.style.setProperty('--border-pulse-duration', '4s'); updateUI(); gameLoop(); }
    ...
    function update() { if (pendingDirection) { direction = pendingDirection; pendingDirection = null; } if (direction.x === 0 && direction.y === 0) return; const currentHead = snake[0]; const newHead = { x: currentHead.x + direction.x, y: currentHead.y + direction.y, color: currentHead.color }; if (newHead.x < 0) newHead.x = TILE_COUNT - 1; else if (newHead.x >= TILE_COUNT) newHead.x = 0; if (newHead.y < 0) newHead.y = TILE_COUNT - 1; else if (newHead.y >= TILE_COUNT) newHead.y = 0; snake.unshift(newHead); if (newHead.x === food.x && newHead.y === food.y) { playSound('eat'); score += 1 * level; snake[0].color = food.color; placeFood(); checkSpeedIncrease(); if (Math.random() < 0.2) placeBonusFood(); } else if (bonusFood && newHead.x === bonusFood.x && newHead.y === bonusFood.y) { playSound('bonus'); const timeRemaining = BONUS_FOOD_DURATION - (Date.now() - bonusFoodSpawnTime); score += Math.max(1, Math.floor((timeRemaining / 1000) * level * 3)); snake[0].color = bonusFood.color; bonusFood = null; clearTimeout(bonusFoodTimeout); checkSpeedIncrease(); } else { snake.pop(); } updateUI(); checkCollision(); }
    function checkCollision() { const head = snake[0]; for (let i = 1; i < snake.length; i++) { if (head.x === snake[i].x && head.y === snake[i].y) { gameOver(); return; } } }
